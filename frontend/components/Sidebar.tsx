...
export function Sidebar({
  ...
  return (
    <aside className="w-full md:w-80 shrink-0 border-r border-hairline bg-card p-4 flex flex-col gap-1 overflow-y-auto">
      ...
      <input
        ...
        disabled={ingesting}
        className="border border-hairline rounded-lg px-3 py-2 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-accent/40"
      />
      <button
        type="button"
      ...
      {repos.length === 0 ? (
        ...
        <>
          <select
            ...
            className="border border-hairline rounded-lg px-3 py-2 text-sm mb-2 bg-card"
          >
            {repos.map((r) => (
          ...
          {selectedCollection &&
            (() => {
              ...
              if (!info) return null;
              return (
                <div className="border border-hairline rounded-[10px] px-3 py-2 text-sm mb-1">
                  <div className="font-semibold">{info.display_name}</div>
